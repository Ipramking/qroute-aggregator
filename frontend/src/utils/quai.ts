export const getZoneForAddress = (address: string): string => {
  ...
  if (prefixByte >= 0x5a && prefixByte <= 0x77) return "ethiopia-1";
  if (prefixByte >= 0x78 && prefixByte <= 0x95) return "ethiopia-2";
  if (prefixByte >= 0x96 && prefixByte <= 0xb3) return "ethiopia-3";
  if (prefixByte >= 0xb4 && prefixByte <= 0xd1) return "paxos-1";
  if (prefixByte >= 0xd2 && prefixByte <= 0xef) return "paxos-2";
  if (prefixByte >= 0xf0 && prefixByte <= 0xff) return "paxos-3";
